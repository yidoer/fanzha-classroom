...
- `releases/fanzha-classroom-0.7.2-washi-update-proxy-spinner-debug.apk`：当前本地验收版，包含和纸界面、止损步骤卡、异步“检查更新”按钮加载态、多源更新重试、GitHub/Cloudflare 下载加速与 SHA-256 校验。
...
开源发布使用 GitHub Actions 校验和构建，GitHub Releases 承载 APK 与剧情包；发布完成后工作流自动生成 SHA-256 清单并回写 `main`。Cloudflare Worker 已部署在 `https://fanzha-story-edge.yidoer.workers.dev`，提供清单缓存、GitHub Release 受限下载代理、匿名上下文压缩与作者候选剧情生成，失败时客户端自动回退 GitHub。详见 `story-pack/README.md` 和 `cloudflare/README.md`。
